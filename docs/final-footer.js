(() => {
  ...
  const setupHeaderControls = () => {
    ...
    if (!languageSwitcher.querySelector('.vei-language-toggle')) {
      ...
      toggle.addEventListener('click', (event) => {
        ...
        const downloadMenu = document.querySelector('.vei-download-wrap');
    ...
    if (!document.querySelector('.vei-download-wrap')) {
      ...
      download.innerHTML = `Descargar <span class="vei-download-chevron">⌄</span>`;

      const menu = document.createElement('div');
      menu.className = 'vei-download-menu';
      menu.innerHTML = `
        <div class="vei-download-title">Disponible para</div>
        <div class="vei-download-grid">
          <div class="vei-download-item is-static"><img src="assets/platforms/windows.webp" alt="Windows"><span>Windows</span></div>
          <a class="vei-download-item" href="/download/VeiCloudVPN.apk" download="VeiCloudVPN.apk"><img src="assets/platforms/android.webp.png" alt="Android"><span>Android</span></a>
          <a class="vei-download-item" href="/download/VeiCloudVPN.apk" download="VeiCloudVPN.apk"><img src="assets/platforms/android-tv.webp.png" alt="Android TV"><span>Android TV</span></a>
          <div class="vei-download-item is-static"><img src="assets/platforms/chrome.webp.png" alt="Chrome"><span>Chrome</span></div>
          <div class="vei-download-item is-static"><img src="assets/platforms/firefox.webp.png" alt="Firefox"><span>Firefox</span></div>
          <div class="vei-download-item is-static"><img src="assets/platforms/edge.webp.png" alt="Edge"><span>Edge</span></div>
        </div>`;
      ...
      const toggleDownload = (event) => {
        ...
        languageSwitcher.classList.remove('is-open');
        download.setAttribute('aria-expanded', open ? 'true' : 'false');
      };

      download.addEventListener('click', toggleDownload);
      download.addEventListener('keydown', (event) => {
        if (event.key === 'Enter' || event.key === ' ') toggleDownload(event);
      });
    }
    ...
    if (!document.documentElement.dataset.veiHeaderGlobalListeners) {
      ...
      document.addEventListener('click', (event) => {
        ...
        const downloadWrap = document.querySelector('.vei-download-wrap');
        if (language && !language.contains(event.target)) language.classList.remove('is-open');
        if (downloadWrap && !downloadWrap.contains(event.target)) downloadWrap.classList.remove('is-open');
      });
      document.addEventListener('keydown', (event) => {
        if (event.key !== 'Escape') return;
        document.querySelector('.language-switcher')?.classList.remove('is-open');
    ...
    if (!document.querySelector('#vei-header-controls-styles')) {
      ...
      style.textContent = `
        ...
        .vei-language-option[data-lang="es"]{top:54px!important}.vei-language-option[data-lang="ru"]{top:100px!important}.vei-language-option[data-lang="en"]{top:146px!important}
        ...
        .vei-language-option:hover{background:#111f2b!important;color:#fff!important}.vei-language-option.active{color:#ff8069!important;background:rgba(255,67,37,.07)!important}
        .vei-download-wrap{position:relative;display:flex;align-items:center}
        .nav-download.vei-download-toggle{background:transparent!important;border:0!important;box-shadow:none!important;padding:8px 0!important;border-radius:0!important;color:#fff!important;font:700 13px Manrope,sans-serif!important;cursor:pointer!important;white-space:nowrap;overflow:visible!important}
        .nav-download.vei-download-toggle:hover{color:#ff8069!important;transform:none!important}.vei-download-chevron{display:inline-block;margin-left:5px;font-size:14px;transition:transform .18s ease}.vei-download-wrap.is-open .vei-download-chevron{transform:rotate(180deg)}
        .vei-download-menu{position:absolute;right:0;top:48px;width:520px;padding:22px;border-radius:22px;background:#0b141d;border:1px solid rgba(255,255,255,.07);box-shadow:0 28px 70px rgba(0,0,0,.52);opacity:0;visibility:hidden;pointer-events:none;transform:translateY(-8px) scale(.985);transform-origin:top right;transition:opacity .17s ease,transform .17s ease,visibility .17s;z-index:120}
        .vei-download-wrap.is-open .vei-download-menu{opacity:1;visibility:visible;pointer-events:auto;transform:translateY(0) scale(1)}
        .vei-download-title{font:800 10px Manrope,sans-serif;letter-spacing:1.5px;text-transform:uppercase;color:#ff8069;margin:2px 4px 16px}
        .vei-download-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:10px}
        .vei-download-item{min-height:74px;border-radius:16px;background:#101c27;display:flex;align-items:center;gap:12px;padding:14px;color:#eaf0f5;transition:background .16s ease,transform .16s ease}
        .vei-download-item:not(.is-static):hover{background:#152634;transform:translateY(-1px)}.vei-download-item.is-static{cursor:default}
        .vei-download-item img{width:28px;height:28px;object-fit:contain;flex:0 0 auto}.vei-download-item span{font:700 12px Manrope,sans-serif}
        @media(max-width:720px){.nav-tools{gap:11px!important}.account-nav-button.vei-account-link,.nav-download.vei-download-toggle{font-size:12px!important}.vei-download-menu{right:-8px;width:min(92vw,430px)}.vei-download-grid{grid-template-columns:repeat(2,1fr)}.language-switcher::after{right:-4px;width:205px}.vei-language-option{width:185px!important}}
        @media(max-width:430px){.vei-download-menu{padding:16px}.vei-download-item{min-height:64px;padding:11px}.vei-download-item img{width:24px;height:24px}.vei-download-item span{font-size:11px}}
      `;
  ...
  const buildFinalFooter = () => {
    ...
    if (nav) nav.innerHTML = `<a href="#home">Inicio</a><a href="#devices">Dispositivos</a><a href="#compare">Comparar</a><a href="#faq">FAQ</a>`;

    const explore = document.querySelector('.vei-hero-actions .ghost');
    if (explore) { explore.href = '#devices'; explore.textContent = 'Explorar VeiCloud'; }

    footer.className = 'vei-final-footer';
    footer.innerHTML = `
      <div class="vei-footer-inner">
        <div class="vei-footer-top"><a class="vei-footer-brand" href="#home" aria-label="VeiCloud VPN"><img class="vei-footer-logo-img" src="assets/veicloud-logo.png" alt="VeiCloud"><span><strong>VeiCloud</strong><small>VPN</small></span></a><p>Privacidad, velocidad y una experiencia sencilla en tus dispositivos compatibles.</p></div>
        <div class="vei-footer-columns">
          <div class="vei-footer-column"><strong>VeiCloud</strong><a href="#home">Inicio</a><a href="#devices">Dispositivos</a><a href="#compare">VeiCloud vs VPN gratuita</a><a href="/download/VeiCloudVPN.apk" download="VeiCloudVPN.apk">Descargar para Android</a></div>
          <div class="vei-footer-column"><strong>Ayuda</strong><a href="#faq">Preguntas frecuentes</a><a href="contacts.html">Contacto y soporte</a><a href="portal.html">Mi cuenta</a></div>
          <div class="vei-footer-column"><strong>Legal</strong><a href="offer.html">Oferta pública</a><a href="privacy.html">Política de privacidad</a><a href="refund.html">Cancelación y reembolsos</a><a href="contacts.html">Información de contacto</a></div>
        </div>
        ...
        <div class="vei-footer-bottom"><span>© 2026 VeiCloud VPN</span><span>Servicio digital · Sin entrega física</span></div>
      </div>`;
