(() => {
  ...
  async function createSbpPaymentAndRedirect(modal, button) {
    ...
    const errorBox = modal.querySelector("#sbpPaymentError");
    errorBox?.classList.add("hidden");
    const originalHtml = button.innerHTML;
    button.disabled = true;
    const copy = button.querySelector(".payment-method-copy");
    if (copy) copy.innerHTML = `<strong>Создание платежа…</strong><small>Подождите несколько секунд</small>`;

    try {
      const detail = selectedPlanDetail(modal);
      const response = await fetch(`${API_BASE}/api/payments/sbp/create`, {
        method: "POST",
        headers: {
        ...
        body: JSON.stringify({ plan: detail.plan })
      });

      let payload = null;
      try { payload = await response.json(); } catch {}

      if (!response.ok) throw new Error(String(extractApiError(payload, "Не удалось создать платёж СБП.")));
      if (!payload?.pay_url) throw new Error("RollyPay payment link was not generated.");

      location.href = payload.pay_url;
    } catch (error) {
      button.innerHTML = originalHtml;
      button.disabled = false;
      if (errorBox) {
        errorBox.textContent = error.message || "Не удалось создать платёж. Попробуйте ещё раз.";
  ...
  async function createInvoiceAndRedirect(modal, button) {
    ...
    try {
      ...
      if (!response.ok) throw new Error(String(extractApiError(payload, "No se pudo crear la factura.")));
  ...
  function ensurePaymentModal() {
        ...
        <div class="payment-stage-window"><div class="payment-stage-track" id="paymentStageTrack">
          <div class="payment-stage">
            ...
            <div class="payment-method-list">
              <button class="payment-method-option" id="sbpPaymentsOption" type="button"><span class="payment-method-icon sbp-icon"><img src="https://sbp.nspk.ru/images/stub/logo.svg" alt="СБП"></span><span class="payment-method-copy"><strong>СБП</strong><small>Оплата через Систему быстрых платежей</small></span><span class="payment-method-arrow">›</span></button>
            ...
            <div class="payment-error hidden" id="sbpPaymentError"></div>
          ...
          <div class="payment-stage">
            ...
            <div class="payment-error hidden" id="cryptoPaymentError"></div>
    ...
    modal.querySelector("#sbpPaymentsOption")?.addEventListener("click", (event) => createSbpPaymentAndRedirect(modal, event.currentTarget));
  ...
  function openPaymentModal(index) {
    ...
    modal.querySelector("#sbpPaymentError")?.classList.add("hidden");
    modal.querySelector("#cryptoPaymentError")?.classList.add("hidden");
    const sbpButton = modal.querySelector("#sbpPaymentsOption");
    if (sbpButton) sbpButton.disabled = false;
    modal.querySelectorAll("[data-crypto-currency]").forEach((item) => { item.disabled = false; });
