(() => {
  ...
  function ensurePaymentModal() {
    ...
    modal.querySelectorAll("[data-crypto-currency]").forEach((button) => {
      button.addEventListener("click", () => {
        modal.querySelectorAll("[data-crypto-currency]").forEach((item) => item.classList.remove("selected"));
        button.classList.add("selected");
        const detail = selectedPlanDetail(modal);
        window.dispatchEvent(new CustomEvent("veicloud:crypto-currency-selected", { detail: { ...detail, currency: button.dataset.cryptoCurrency, network: button.dataset.cryptoNetwork } }));
      });
    });
    return modal;
  }
